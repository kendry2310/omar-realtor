...
def search_one(query, aspect=None, size=None, strict=False):
    """Search Openverse, progressively relaxing filters until we find something.
    If strict=True, NEVER drop the license filter (commercially-safe only)."""
    page_size = 6
    tries = [
    ...
    if not strict:
        tries.append({"q": query, "mature": "false", "page_size": page_size})
    for params in tries:
...
def main():
    ...
    parser.add_argument("--strict-license", action="store_true", help="Never fall back to unfiltered license search")
    ...
    # Merge with existing credits instead of overwriting
    credits = {}
    ...
    for item in queries:
        ...
        result = search_one(query, aspect=aspect, size=size, strict=args.strict_license)
        ...
        lic = result.get("license") or "cc"
        log(f" OK {size_bytes // 1024} KB [{lic}] - {clean_title(result.get('title', ''))[:40]}")
